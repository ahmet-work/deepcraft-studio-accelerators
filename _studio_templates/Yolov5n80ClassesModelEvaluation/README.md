# YOLOv5 Nano (Updated) Evaluation

## Model Description

**YOLOv5nu** is the updated version of YOLOv5 Nano, featuring an **anchor-free detection head** (indicated by the "u" suffix). This modernized variant replaces the original anchor-based detection with the anchor-free approach introduced in later YOLO versions, providing improved detection accuracy and simpler post-processing while preserving YOLOv5's proven backbone and neck architecture.

- **Architecture**: YOLOv5 (Ultralytics, updated with anchor-free head)
...
| File | Description |
|------|-------------|
| `Model/yolov5nu.pt` | Original PyTorch model with anchor-free head (~5.3 MB) |
| `Model/yolov5n_int8.tflite` | INT8 quantized TFLite model for edge deployment (~2.8 MB) |
| `yolov5n_int8.improjv` | DEEPCRAFT™ Studio project file |
| `yolov5n_int8.imunit` | DEEPCRAFT™ Studio processing graph for live camera evaluation |
| `README.md` | This file |
