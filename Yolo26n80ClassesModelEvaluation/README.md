# YOLO26 Nano Evaluation

## Model Description

**YOLO26n** (YOLO26 Nano) represents a newer iteration in the YOLO object detection model family. Building on the advances of previous YOLO generations, it delivers improved accuracy-efficiency trade-offs while remaining compact enough for resource-constrained edge devices.

- **Architecture**: YOLO26 (Ultralytics)
...
| File | Description |
|------|-------------|
| `Model/yolo26n.pt` | Original PyTorch model (~5.3 MB) |
| `Model/yolo26n_int8.tflite` | INT8 quantized TFLite model for edge deployment (~2.6 MB) |
| `yolo26n_int8.improjv` | DEEPCRAFT Studio project file |
| `yolo26n_int8.imunit` | DEEPCRAFT Studio processing graph for live camera evaluation |
| `README.md` | This file |
...
```
Camera (640×360 @ 15fps) → Resize (224×224) → Cast (int8) → YOLO26n TFLite → Dequantize (float32) → Select BBox + Classes → Detection Filter (threshold: 0.3) → Labels → Bounding Box Visualization
```
...
1. Open `yolo26n_int8.imunit` from the Solution Explorer.
